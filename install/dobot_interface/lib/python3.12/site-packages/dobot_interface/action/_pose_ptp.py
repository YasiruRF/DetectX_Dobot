...
class PosePTP_Feedback(metaclass=Metaclass_PosePTP_Feedback):
    ...
    def __init__(self, **kwargs):
        ...
        if self._check_fields:
            assert all('_' + key in self.__slots__ for key in kwargs.keys()), \
                'Invalid arguments passed to constructor: %s' % \
                ', '.join(sorted(k for k in kwargs.keys() if '_' + k not in self.__slots__))
        if 'present_pose' not in kwargs:
            self.present_pose = numpy.zeros(4, dtype=numpy.float32)
        else:
            self.present_pose = kwargs.get('present_pose')
    ...
    def __eq__(self, other):
        if not isinstance(other, self.__class__):
            return False
        if any(self.present_pose != other.present_pose):
            return False
        return True
    ...
    @builtins.property
    def present_pose(self):
        """Message field 'present_pose'."""
        return self._present_pose

    @present_pose.setter
    def present_pose(self, value):
        if self._check_fields:
            if isinstance(value, numpy.ndarray):
                assert value.dtype == numpy.float32, \
                    "The 'present_pose' numpy.ndarray() must have the dtype of 'numpy.float32'"
                assert value.size == 4, \
                    "The 'present_pose' numpy.ndarray() must have a size of 4"
                self._present_pose = value
                return
            from collections.abc import Sequence
            from collections.abc import Set
            from collections import UserList
            ...
            assert \
                ((isinstance(value, Sequence) or
                 ...
                 all(isinstance(v, float) for v in value) and
                 all(not (val < -3.402823466e+38 or val > 3.402823466e+38) or math.isinf(val) for val in value)), \
                "The 'present_pose' field must be a set or sequence with length 4 and each value of type 'float' and each float in [-340282346600000016151267322115014000640.000000, 340282346600000016151267322115014000640.000000]"
        self._present_pose = numpy.array(value, dtype=numpy.float32)
